    ...
    Class RESTAPIModel{
        ...
        function getAllQuotationComments($id){
            ...
            try {
                $Dbobj = new DbConnection();
                $conn = $Dbobj->getdbconnect();
                $query = mysqli_query($conn, "SELECT qc.id, qc.user_id, u.role, u.name as user_name, comments, created_on as date_time FROM quotation_comments qc INNER JOIN users u ON qc.user_id = u.id WHERE qc.quotation_id = ".$id);
                $count  = mysqli_num_rows($query);
                if ($count > 0) {
                    while($row = mysqli_fetch_assoc($query)) {
                        $comments[] = $row;
