    ...
    Class RESTAPIModel{
        ...
        function validatePin($pin){
            ...
            try {
                $Dbobj = new DbConnection();
                $conn = $Dbobj->getdbconnect();
                $query = mysqli_query($conn, "SELECT * FROM users WHERE (password='".$pin."') AND active='1' AND is_expired='0'");
                $user = mysqli_fetch_assoc($query);
            } catch (Exception $e) {
                print "Error!: " . $e->getMessage() . "<br/>";
                die();
        ...
        function approveQuotation($id,$approved_price,$approved_by=0){
            ...
            try {
                ...
                $sql = "UPDATE quotations SET approved_price='".$approved_price."', approved_by='".$approved_by."', approved_date=DATE('".$currentDate."'), status = '1' WHERE id = '" . $id . "'";
                $query = mysqli_query($conn, $sql);
                $count  = mysqli_affected_rows($conn);
                if($count>0){
                    $user = $this->getPushTokenByUserID($user_id);
        ...
        function rejectQuotation($id,$dropped_by=0){
            ...
            try {
                ...
                $conn = $Dbobj->getdbconnect();
                $currentDate = date("Y/m/d");
                $sql = "UPDATE quotations SET dropped_by='".$dropped_by."', dropped_date=DATE('".$currentDate."'), status = '2' WHERE id = '" . $id . "'";
                $query = mysqli_query($conn, $sql);
                $count  = mysqli_affected_rows($conn);
                if($count>0){
                    $user = $this->getPushTokenByUserID($user_id);
        ...
        function getAllQuotations($user_id='',$sort_by='',$date_from='',$date_to='',$price_min='',$price_max=''){
            ...
            try {
                ...
                $sql = "SELECT q.id, q.user_id, u.name,u.role,u.company, q.make_id, q.make_display, q.model_id, q.model_display, q.year_id, q.year, q.variant_id, q.variant_display, q.car_color, q.fuel_type, q.car_kms, q.car_owner, q.is_replacement, q.structural_damage, q.structural_damage_desc, q.insurance_date, q.refurbishment_cost, q.requested_price, q.approved_price,IFNULL((SELECT au.name FROM users au WHERE au.id = q.approved_by LIMIT 1),'-') as approved_by,DATE_FORMAT(q.approved_date, '%d %b %Y') as approved_date, IFNULL((SELECT du.name FROM users du WHERE du.id = q.dropped_by LIMIT 1),'-') as dropped_by, DATE_FORMAT(q.dropped_date, '%d %b %Y') as dropped_date, CASE WHEN q.status = '0' THEN 'Pending' WHEN q.status = '1' THEN 'Approved' ELSE 'Rejected' END AS status,DATE_FORMAT(q.created_on, '%d %b %Y') as created_date FROM quotations q INNER JOIN users u ON q.user_id = u.id WHERE q.status IN (0,1,2)".$condition;
                $query = mysqli_query($conn, $sql);
                $count  = mysqli_num_rows($query);
                if ($count > 0) {
                    while($row = mysqli_fetch_assoc($query)) {
        ...
        function getQuotationDetail($id){
            ...
            try {
                $Dbobj = new DbConnection();
                $conn = $Dbobj->getdbconnect();
                $query = mysqli_query($conn, "SELECT q.id, q.user_id, u.name,u.role,u.company, make_id, make_display, model_id, model_display, year_id, year, variant_id, variant_display, car_color, fuel_type, car_kms, car_owner, is_replacement,structural_damage,structural_damage_desc, insurance_date, refurbishment_cost, requested_price, approved_price, IFNULL((SELECT au.name FROM users au WHERE au.id = q.approved_by LIMIT 1),'-') as approved_by,DATE_FORMAT(q.approved_date, '%d %b %Y') as approved_date, IFNULL((SELECT du.name FROM users du WHERE du.id = q.dropped_by LIMIT 1),'-') as dropped_by,DATE_FORMAT(q.dropped_date, '%d %b %Y') as dropped_date, CASE WHEN status = '0' THEN 'Pending' WHEN status = '1' THEN 'Approved' ELSE 'Rejected' END AS status,DATE_FORMAT(q.created_on, '%d %b %Y') as created_date FROM quotations q INNER JOIN users u ON q.user_id = u.id WHERE q.id = ".$id);
