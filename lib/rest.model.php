    ...
    Class RESTAPIModel{
        ...
        function getQuotationDetail($id){
            ...
            try {
                $Dbobj = new DbConnection();
                $conn = $Dbobj->getdbconnect();
                $query = mysqli_query($conn, "SELECT q.id, q.user_id, u.name,u.role,u.company, make_id, make_display, model_id, model_display, year_id, year, variant_id, variant_display, car_color, fuel_type, car_kms, car_owner, is_replacement,structural_damage,structural_damage_desc, insurance_date, refurbishment_cost, requested_price, approved_price, IFNULL((SELECT au.name FROM users au WHERE au.id = q.approved_by LIMIT 1),'-') as approved_by, q.approved_date, IFNULL((SELECT du.name FROM users du WHERE du.id = q.dropped_by LIMIT 1),'-') as dropped_by, q.dropped_date, CASE WHEN status = '0' THEN 'Pending' WHEN status = '1' THEN 'Approved' ELSE 'Rejected' END AS status,DATE_FORMAT(q.created_on, '%d %M %Y') as created_date FROM quotations q INNER JOIN users u ON q.user_id = u.id WHERE q.id = ".$id);
