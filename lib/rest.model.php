    ...
    Class RESTAPIModel{
        ...
        function getAllAgentsList(){
            ...
            try {
                $Dbobj = new DbConnection();
                $conn = $Dbobj->getdbconnect();
                $query = mysqli_query($conn, "SELECT id,name,mobile,email,company,location,designation,role,password as pin,is_expired,active,token FROM users ORDER BY role ASC, name ASC, active DESC");
                $count  = mysqli_num_rows($query);
                if ($count > 0) {
                    while($row = mysqli_fetch_assoc($query)) {
                        $agents[] = $row;
